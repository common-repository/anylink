...
class al_option {
	...
	public function anyLinkSettingPage() {
		...
		require_once( ANYLNK_PATH . '/al_setting.php' );
	}
	public function alAdminInit() {
        ...
        add_settings_field(  'filter_comment_toggle', __( 'Turn on comment filter.', 'anylink' ), array( $this, 'display_comment_toggle'), 'anyLinkSetting', 'al_general_settings' );
    ...
    public function add_meta_box(){
        add_meta_box( 'anylink_filter_toggle', ' ', array( $this, 'render_meta_box_content' ), 'post', 'side', 'high' );
    }
    
    /**
     * Echo anylink toggle checkbox
     * 
     * @param mixed $post
     * @return void
     */
    public function render_meta_box_content( $post ){
        wp_nonce_field( 'anylink_inner_custom_box', 'anylink_toggle_nonce' );
        $value = get_post_meta( $post -> ID, '_anylink_toggle', true );
        if( (boolean)$value ){
            $checked = 'checked="checked"';
        } else {
            $checked = '';
        }
        echo "<input type='checkbox' id='anylink_toggle' name='anylink_toggle' {$checked}/>";
        echo '<label for="anylink_toggle">'. __( 'Do not apply anylink plugin for this post.', 'anylink' ) .'</label>';
    }
    public function save( $post_id ){
        if( ! isset( $_POST['anylink_toggle_nonce'] ) ) {
            return $post_id;
        }
        $nonce = $_POST['anylink_toggle_nonce'] ;
        if( !wp_verify_nonce( $nonce, 'anylink_inner_custom_box' ) ){
            return $post_id;
        }
        ...
        $mydata = ( isset( $_POST['anylink_toggle'] ) ) ? sanitize_text_field( $_POST['anylink_toggle'] ) : 0;
        update_post_meta( $post_id, '_anylink_toggle', $mydata );
    }
}
?>