...
function anylnkInstall() {
	...
	if( ! get_option( 'anylink_options' ) ) {
        ...
        if( (float)$al_option['version'] < 30 ) {
            $al_option['rules'] = '';
            $al_option['version'] = 30;
            update_option( 'anylink_options', $al_option );
        }
	}		
}
...
/**
 * Trim "http://www." in urls
 * 
 * @param $url mixed $url
 * @return $url trimed url
 */
function al_trim_http( $url ){
    if( indexOf( 'http:', $url ) )
        $url = substr( $url, 7 );
    elseif( indexOf( 'https:', $url ) )
        $url = substr( $url, 8 );
    if( indexOf( 'www.', $url ) )
        $url = substr( $url, 4 );
    return $url;
